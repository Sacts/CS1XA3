...
def signup_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            newpassword=form.cleaned_data.get('password1')
            models.UserInfo.objects.create_user_info(username=newusername, password=newpassword)
    ...
    form=UserCreationForm(request.POST)
    failed=request.session.get('failed1',False)

    context = { 'signup_form' : form, 'failed1' : failed}
